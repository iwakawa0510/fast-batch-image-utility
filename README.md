...
本ツールは、CSP（CLIP STUDIO PAINT）,PHOTOSHOP,TVpaint等の画像編集ツールの前後で使用する外部ユーティリティです。大量の画像に対する単純な変換処理を高速に実行することを目的としています。
※動作確認はCSPでのみ行っております。
### 主な特徴
...
## コーディング支援とLLM(生成AI)について
- このプロジェクトはChatGPT5.2で生成された仕様書からClaude(Sonnet4.5)を使って生成されたソースコードが多分に含まれます。
- またGemini2.5,3.0をリファクタリングに使用しています。
- これらには極めて強い拘束を課したのでAI贋作やハルシネーションは著者には発見出来ませんでした。もし該当部分があれば修正対応を行います。
---
